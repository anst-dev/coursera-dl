...
from cs_dlp.libs.define import (
    IN_MEMORY_EXTENSION,
    IN_MEMORY_MARKER,
    ...
    OPENCOURSE_UNGRADED_WIDGET_SESSIONS_V1_URL,
...
from cs_dlp.libs.network import get_page, get_reply, post_page_and_reply
from cs_dlp.libs.utils import (
    BeautifulSoup,
    clean_filename,
    clean_url,
...
class QuizExamToMarkupConverter(object):
    ...
    def __call__(self, quiz_or_exam_json):
        ...
        for question_index, question_json in enumerate(quiz_or_exam_json["questions"]):
            ...
            if question_type not in self.KNOWN_QUESTION_TYPES:
                ...
                logging.info("Please report class name, quiz name and the data above to coursera-dl authors")
            ...
            input_type = {"mcq": "radio", "mcqReflect": "radio", "checkbox": "checkbox"}.get(question_type, "")

            # Convert options, they are either checkboxes or radio buttons
            result.extend(self._convert_options(question_index, options, input_type))
    ...
    def _convert_options(self, question_index, options, input_type):
        ...
        for option in options:
            ...
            result.append(
                '<label><input type="%s" name="%s">%s<br></label>' % (input_type, question_index, option_text)
            )
    ...
    def _generate_input_field(self):
        return ['<form><label>Enter answer here:<input type="text" name=""><br></label></form>']
...
class MarkupToHTMLConverter(object):
    ...
    def _convert_markup_images(self, soup):
        ...
        images = [image for image in soup.find_all("img") if image.attrs.get("assetid") is not None]
        if not images:
            return
    ...
    def _convert_markup_audios(self, soup):
        ...
        audios = [
            ...
            if audio.attrs.get("id") is not None and audio.attrs.get("assettype") == "audio"
        ]
        if not audios:
            return
...
class OnDemandCourseMaterialItemsV1(object):
    ...
    def create(session, course_name):
        ...
        dom = get_page(session, OPENCOURSE_ONDEMAND_COURSE_MATERIALS_V2, json=True, class_name=course_name)
        return OnDemandCourseMaterialItemsV1(dom["linked"]["onDemandCourseMaterialItems.v2"])
...
class AssetRetriever(object):
    ...
    def __call__(self, asset_ids, download=True):
        ...
        asset_list = get_page(self._session, OPENCOURSE_API_ASSETS_V1_URL, json=True, id=",".join(asset_ids))
...
class ModulesV1(object):
    ...
    def from_json(data):
        return ModulesV1(
            OrderedDict(
                (item["id"], ModuleV1(item["name"], item["id"], item["slug"], item["lessonIds"])) for item in data
            )
        )
...
class LessonsV1(object):
    ...
    def from_json(data):
        return LessonsV1(
            OrderedDict(
                (item["id"], LessonV1(item["name"], item["id"], item["slug"], item["itemIds"])) for item in data
            )
        )
...
class CourseraOnDemand(object):
    ...
    def __init__(self, session, course_id, course_name, unrestricted_filenames=False, mathjax_cdn_url=None):
        """
    ...
    def _get_notebook_folder(self, url, jupyterId, **kwargs):
        ...
        for content in reply["content"]:
            if content["type"] == "directory":
                ...
                r = requests.get(tmp_url.replace(" ", "%20"), cookies=self._session.cookies)
                ...
                supplement_links[str(extension[1:])].append((tmp_url.replace(" ", "%20"), filename))
                ...
                r = requests.get(tmp_url.replace(" ", "%20"), cookies=self._session.cookies)
                if not os.path.exists(self._course_name + "/notebook/" + head + "/" + tail):
                    logging.info("Downloading Jupyter %s into %s", tail, head)
                    with open(self._course_name + "/notebook/" + head + "/" + tail, "wb+") as f:
                        f.write(r.content)
                else:
                    logging.info("Skipping %s... (file exists)", tail)
                ...
                supplement_links["ipynb"].append((tmp_url.replace(" ", "%20"), filename))
    ...
    def _get_notebook_json(self, notebook_id, authorizationId):
        ...
        req = newReq.get(
            OPENCOURSE_NOTEBOOK_TREE.format(jupId=jupyted_id, path="/", timestamp=int(time.time())),
            headers=headers,
        )

        return self._get_notebook_folder(
            OPENCOURSE_NOTEBOOK_TREE, jupyted_id, jupId=jupyted_id, path="/", timestamp=int(time.time())
        )
    ...
    def _convert_quiz_json_to_links(self, quiz_json, filename_suffix):
        ...
        extend_supplement_links(supplement_links, {IN_MEMORY_EXTENSION: [instructions]})
        return supplement_links
    ...
    def _get_exam_session_id(self, exam_id):
        ...
        _body, reply = post_page_and_reply(
            self._session, POST_OPENCOURSE_ONDEMAND_EXAM_SESSIONS, data=json.dumps(data), headers=headers
        )
    ...
    def extract_links_from_lecture(self, course_id, video_id, subtitle_language="en", resolution="540p"):
        """
        ...
        """
        try:
            links = self._extract_videos_and_subtitles_from_lecture(course_id, video_id, subtitle_language, resolution)

            assets = self._get_lecture_asset_ids(course_id, video_id)
            assets = self._normalize_assets(assets)
            extend_supplement_links(links, self._extract_links_from_lecture_assets(assets))
    ...
    def _get_lecture_asset_ids(self, course_id, video_id):
        ...
        # be extracted later.
        return [asset["id"] for asset in dom["linked"]["openCourseAssets.v1"]]
    ...
    def _extract_links_from_lecture_assets(self, asset_ids):
        ...
        def _add_asset(name, url, destination):
            ...
            extension = clean_filename(extension.lower().strip(".").strip(), self._unrestricted_filenames)
    ...
    def _get_asset_urls(self, asset_id):
        ...
        """
        dom = get_page(self._session, OPENCOURSE_ASSETS_URL, json=True, id=asset_id)
        ...
        for element in dom["elements"]:
            ...
            if typeName == "asset":
                ...
                for asset in self._asset_retriever([open_course_asset_id], download=False):
            ...
            #
                ...
                urls.append({"name": definition["name"].strip(), "url": definition["url"].strip()})
                ...
                logging.warning(
                    ...
                    typeName,
                    json.dumps(dom, indent=4),
                )

        return urls

    def _extract_videos_and_subtitles_from_lecture(
        self, course_id, video_id, subtitle_language="en", resolution="540p"
    ):
        logging.debug("Parsing JSON for video_id <%s>.", video_id)

        dom = get_page(
            self._session, OPENCOURSE_ONDEMAND_LECTURE_VIDEOS_URL, json=True, course_id=course_id, video_id=video_id
        )
        ...
        if resolution in videos:
            ...
            logging.debug("Proceeding with download of resolution %s of <%s>.", resolution, video_id)
        else:
        ...
        subtitle_link = self._extract_subtitles_from_video_dom(dom, subtitle_language, video_id)
    ...
    def _extract_subtitles_from_video_dom(self, video_dom, subtitle_language, video_id):
        ...
        for subtitle_node, subtitle_extension, subtitle_description in subtitle_nodes:
            ...
            if subtitles is not None:
                ...
                for language_with_alts in requested_subtitle_list:
                    if download_all_subtitle:
                        break
                    grouped_language_list = [l.strip() for l in language_with_alts.split("|")]
            ...
            if not download_all_subtitle and subtitle_set_nonexist:
                logging.warning(
                    "%s unavailable in '%s' language for video with video id: [%s],%s",
                    subtitle_description.capitalize(),
                    ", ".join(subtitle_set_nonexist),
                    video_id,
                    subtitle_description,
                )
            if not subtitle_set_download:
                logging.warning(
                    "%s all requested subtitles are unavailable,with video id: [%s], falling back to 'en' %s",
                    subtitle_description.capitalize(),
                    video_id,
                    subtitle_description,
                )
            ...
            for current_subtitle_language in subtitle_set_download:
                ...
                if subtitle_url is not None:
                    ...
                    subtitle_links["%s.%s" % (current_subtitle_language, subtitle_extension)] = (
                        make_coursera_absolute_url(subtitle_url)
                    )
        return subtitle_links

    def extract_links_from_ungradedWidget(self, element_id):
        """
        Extract Ungraded widget reading content as MD file.
        ...
        Args:
          ...
          Returns:
            dict: Dictionary with supplement links grouped by extension.
            Format: {
                'md': [
                    ('<url1>', '<title1>'),
                    ('<url2>', '<title2>'),
        ...
        json_data = get_page(
            self._session,
            OPENCOURSE_UNGRADED_WIDGET_SESSIONS_V1_URL.format(
                user_id=self._user_id, course_id=self._course_id, element_id=element_id
            ),
            json=True,
        )
        ...
        for element in json_data.get("elements", []):
            ...
            src_url = config.get("src", "")

            if not src_url:
                continue

            # Replace .md.html with .md to get the direct download link
            md_url = src_url.replace(".md.html", ".md")

            # Extract title from URL filename
            filename = os.path.basename(clean_url(md_url))
            title = os.path.splitext(filename)[0] if filename else "ungraded_widget"

            # Group by extension 'md'
            if "md" not in supplement_links:
                supplement_links["md"] = []
            supplement_links["md"].append((md_url, title))
    ...
    def extract_links_from_programming_immediate_instructions(self, element_id):
        ...
        @return: @see CourseraOnDemand._extract_links_from_text
        """
        logging.debug("Extracting links from programming immediate instructions for element_id <%s>.", element_id)

        try:
            # Assignment text (instructions) contains asset tags which describe
            # supplementary files.
            text = "".join(self._extract_programming_immediate_instructions_text(element_id))
            if not text:
                return {}

            supplement_links = self._extract_links_from_text(text)
            instructions = (IN_MEMORY_MARKER + self._markup_to_html(text), "instructions")
            extend_supplement_links(supplement_links, {IN_MEMORY_EXTENSION: [instructions]})
            return supplement_links
        except requests.exceptions.HTTPError as exception:
            logging.error("Could not download programming assignment %s: %s", element_id, exception)
            if is_debug_run():
                logging.exception("Could not download programming assignment %s: %s", element_id, exception)
            return None
    ...
    def extract_links_from_peer_assignment(self, element_id):
        ...
        try:
            ...
            supplement_links = self._extract_links_from_text(text)
            instructions = (IN_MEMORY_MARKER + self._markup_to_html(text), "peer_assignment_instructions")
            extend_supplement_links(supplement_links, {IN_MEMORY_EXTENSION: [instructions]})
            return supplement_links
        except requests.exceptions.HTTPError as exception:
            logging.error("Could not download peer assignment %s: %s", element_id, exception)
            if is_debug_run():
                logging.exception("Could not download peer assignment %s: %s", element_id, exception)
            return None
    ...
    def extract_links_from_supplement(self, element_id):
        ...
        try:
            dom = get_page(
                self._session, OPENCOURSE_SUPPLEMENT_URL, json=True, course_id=self._course_id, element_id=element_id
            )
            ...
            for asset in dom["linked"]["openCourseAssets.v1"]:
                ...
                # both of them.
                extend_supplement_links(supplement_content, self._extract_links_from_text(value))

                instructions = (IN_MEMORY_MARKER + self._markup_to_html(value), "instructions")
                extend_supplement_links(supplement_content, {IN_MEMORY_EXTENSION: [instructions]})

            return supplement_content
        except requests.exceptions.HTTPError as exception:
            logging.error("Could not download supplement %s: %s", element_id, exception)
            if is_debug_run():
                logging.exception("Could not download supplement %s: %s", element_id, exception)
            return None
    ...
    def _extract_asset_tags(self, text):
        ...
        for asset in soup.find_all("asset"):
            asset_tags_map[asset["id"]] = {"name": asset["name"], "extension": asset["extension"]}
    ...
    def _extract_asset_urls(self, asset_ids):
        ...
        """
        dom = get_page(self._session, OPENCOURSE_ASSET_URL, json=True, ids=quote_plus(",".join(asset_ids)))

        return [{"id": element["id"], "url": element["url"].strip()} for element in dom["elements"]]

    def extract_references_poll(self):
        try:
            dom = get_page(self._session, OPENCOURSE_REFERENCES_POLL_URL.format(course_id=self._course_id), json=True)
            logging.info("Downloaded resource poll (%d bytes)", len(dom))
            return dom["elements"]

        except requests.exceptions.HTTPError as exception:
            logging.error("Could not download resource section: %s", exception)
            if is_debug_run():
                logging.exception("Could not download resource section: %s", exception)
            return None
    ...
    def extract_links_from_reference(self, short_id):
        ...
        try:
            dom = get_page(
                self._session, OPENCOURSE_REFERENCE_ITEM_URL, json=True, course_id=self._course_id, short_id=short_id
            )
            ...
            for asset in dom["linked"]["openCourseAssets.v1"]:
                ...
                # and <a href> tags (depending on the course), so we extract
                # both of them.
                extend_supplement_links(resource_content, self._extract_links_from_text(value))

                instructions = (IN_MEMORY_MARKER + self._markup_to_html(value), "resources")
                extend_supplement_links(resource_content, {IN_MEMORY_EXTENSION: [instructions]})

            return resource_content
        except requests.exceptions.HTTPError as exception:
            logging.error("Could not download supplement %s: %s", short_id, exception)
            if is_debug_run():
                logging.exception("Could not download supplement %s: %s", short_id, exception)
            return None
    ...
    def _extract_assignment_text(self, element_id):
        ...
        return [
            element["submissionLearnerSchema"]["definition"]["assignmentInstructions"]["definition"]["value"]
            for element in dom["elements"]
        ]
    ...
    def _extract_links_from_text(self, text):
        ...
        extend_supplement_links(supplement_links, self._extract_links_from_asset_tags_in_text(text))
    ...
    def _extract_links_from_a_tags_in_text(self, text):
        ...
        """
        soup = BeautifulSoup(text)
        links = [item["href"].strip() for item in soup.find_all("a") if "href" in item.attrs]
